...
from rdpower._utils import as_1d_array, as_y_r, current_vce, subset_sample, two_sided_power
...
def rdpower(data=None, cutoff=0, tau=None, alpha=0.05, nsamples=None, sampsi=None, samph=None, all=False,
            ...
            cluster=None, nnmatch=3, scalepar=1, scaleregul=1, sharpbw=False, fuzzy=None, level=95,
            weights=None, masspoints='adjust', bwcheck=None, bwrestrict=True, stdvars=False, subset=None):
    ...
    - vce: Option for rdrobust: specifies the variance-covariance estimator. Current options are 'nn', 'hc0', 'hc1', 'hc2', 'hc3', and, with cluster specified, 'cr1', 'cr2', and 'cr3'.
    - cluster: Option for rdrobust: indicates the cluster ID variable used for cluster-robust variance estimation.
    - nnmatch: Option for rdrobust: minimum number of neighbors for vce='nn'. Default is 3.
    ...
    - sharpbw: Option for rdrobust: uses sharp RD bandwidth selection for fuzzy RD estimation.
    ...
    - stdvars: Option for rdrobust: if True, x and y are standardized before computing the bandwidths. Default is False.
    - subset: Option for rdrobust: optional boolean mask or integer indices specifying the observations to use.
    ...
    if data is not None:
        Y, R = as_y_r(data)
        Y_rd, R_rd, cluster_rd = Y, R, cluster
        Y, R, cluster = subset_sample(Y, R, cluster, subset)
    ...
    if sampsi is not None:
        sampsi = as_1d_array(sampsi)
        if np.any(sampsi % 1 != 0):
    ...
    if data is not None:
        if bias is None or variance is None:
            aux = rdrobust(Y_rd, R_rd, c=cutoff, all=True, covs=covs, covs_drop=covs_drop, deriv=deriv, p=p, q=q, h=h, b=b, rho=rho, cluster=cluster_rd,
                            kernel=kernel, bwselect=bwselect, vce=vce, nnmatch=nnmatch, scalepar=scalepar, scaleregul=scaleregul,
                            sharpbw=sharpbw, fuzzy=fuzzy, level=level, weights=weights, subset=subset, masspoints=masspoints,
    ...
    power_rbc = two_sided_power(tau, se_rbc, z)
    power_conv = two_sided_power(tau + bias, se_conv, z)
    tau_grid = tau * np.array([0, 0.2, 0.5, 0.8])
    power_rbc_list = two_sided_power(tau_grid, se_rbc, z).tolist()
    power_conv_list = two_sided_power(tau_grid + bias, se_conv, z).tolist()
    ...
    if plot:
        ...
        x_grid = np.linspace(left, right, num=100)

        plt.plot(x_grid, two_sided_power(x_grid, se_rbc, z))
        plt.xlabel('tau')
        plt.ylabel('power')
        plt.xlim(left, right)
        ...
        if all:
            plt.plot(x_grid, two_sided_power(x_grid + bias, se_conv, z), linestyle='dashed')
