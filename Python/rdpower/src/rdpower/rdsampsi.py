...
def rdsampsi(data=None,
    ...
    if data is not None:
        if bias is None or variance is None:
            aux = rdrobust(Y_rd, R_rd, c=cutoff, all=True, covs=covs, covs_drop=covs_drop, deriv=deriv, p=p, q=q, h=h, b=b,
                                    rho=rho, kernel=kernel, bwselect=bwselect, vce=vce, cluster=cluster_rd,
                                    nnmatch=nnmatch, scalepar=scalepar, scaleregul=scaleregul, sharpbw=sharpbw,
                                    fuzzy=fuzzy, level=level, weights=weights, subset=subset, masspoints=masspoints,
    ...
    if plot:
        ...
        def power_func(x):
            return two_sided_power(np.sqrt(x * denom) * tau, stilde, z)
        ...
        if all:
            x_grid = np.arange(left, right+1)
            power_conv_grid = two_sided_power(np.sqrt(x_grid * denom_cl) * (tau + bias), stilde_cl, z)
            plt.plot(x_grid, power_conv_grid, label='conventional')
