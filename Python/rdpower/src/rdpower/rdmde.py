...
from rdpower._utils import as_1d_array, as_y_r, current_vce, subset_sample
...
def rdmde(data=None,
    ...
    Parameters:
        ...
        vce: str, optional
            Variance-covariance estimator. Current options are 'nn', 'hc0', 'hc1', 'hc2', 'hc3', and, with cluster specified, 'cr1', 'cr2', and 'cr3'.
        cluster: array-like, optional
            Cluster ID variable used for cluster-robust variance estimation.
        nnmatch: int, optional (default=3)
            Minimum number of neighbors for vce='nn'.
        ...
        sharpbw: bool, optional (default=False)
            Uses sharp RD bandwidth selection for fuzzy RD estimation.
        fuzzy: str, optional
        ...
        stdvars: bool, optional (default=False)
            If True, x and y are standardized before computing the bandwidths.
        subset: array-like, optional
            Boolean mask or integer indices specifying the observations to use.
    ...
    if data is not None:
        if bias is None or variance is None:
            aux = rdrobust(Y_rd, R_rd, c=cutoff, all=True, covs=covs, covs_drop=covs_drop, deriv=deriv, p=p, q=q, h=h, b=b,
                           rho=rho, cluster=cluster_rd, kernel=kernel, bwselect=bwselect, vce=vce, nnmatch=nnmatch,
                           scalepar=scalepar, scaleregul=scaleregul, sharpbw=sharpbw, fuzzy=fuzzy, level=level,
                           weights=weights, subset=subset, masspoints=masspoints, bwcheck=bwcheck,
                           bwrestrict=bwrestrict, stdvars=stdvars)
    ...
    mde_aux = rdpower_powerNR_mde(ntilde, tau0, se_rbc, z, beta)
    mde = mde_aux['mde']
    ...
    for r in [-0.125, -0.0625, 0.0625, 0.125]:
        ...
        if baux < 1:
            rdpow_aux = rdpower_powerNR_mde(ntilde, tau0, se_rbc, z, baux)
            mde_rbc_list[count] = rdpow_aux['mde']

        count += 1

    if all:
        mde_conv_aux = rdpower_powerNR_mde(ntilde, tau0 + bias, se_conv, z, beta)
        ...
        for r in [-0.125, -0.0625, 0.0625, 0.125]:
            ...
            if baux < 1:
                rdpow_conv_aux = rdpower_powerNR_mde(ntilde, tau0 + bias, se_conv, z, baux)
